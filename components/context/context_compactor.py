...
class ContextCompactor:
    ...
    def save_output(self, tool_use_id: str, output: str) -> Path:
        ...
        safe_id = re.sub(r"[^A-Za-z0-9._-]", "_",
                         str(tool_use_id))[:120] or "unknown"
    ...
    # 按顺序执行上下文压缩步骤，返回压缩后的消息列表
    # 1. 持久化过大的工具结果（tool_result_budget），并压缩消息列表，仅保留头尾部分(snip_compact)
    # 2. 如果仍然超出限制，微压缩工具结果
    # 3. 如果仍然超出限制，尝试将工具结果替换为预览
    # 4. 如果仍然超出限制，归档历史并生成摘要
